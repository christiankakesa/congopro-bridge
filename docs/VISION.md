# Congopro Bridge — Long-Term Vision (African Hub | African Bridges | Bridges)

> **Status: aspirational.** Nothing in this document is built. It is the
> multi-country, 13M-company end-state the platform may grow into (NATS
> ingestion, distributed storage, Kubernetes, Telegram Mini App). The system
> that actually runs today is described in [ARCHITECTURE.md](ARCHITECTURE.md),
> and the build order in [BACKEND_PROPOSAL.md](BACKEND_PROPOSAL.md). Revisit
> this document only once the current architecture is genuinely outgrown.
